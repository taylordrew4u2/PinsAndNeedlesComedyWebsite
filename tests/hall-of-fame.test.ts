...
import { hallNavigation, performerLink, publishedPerformers } from "../src/lib/hall-of-fame.ts";
...
test("only named published performers appear, in admin order", () => {
  const people = [
  ...
  assert.deepEqual(publishedPerformers({ ...hall, performers: people }).map((p) => p.id), ["b", "a"]);
});
